		...
		if validateRotationAndRatio(min_rect):
			...
			if(isMaxWhite(plate_img)):
				...
				if rect:
					...
					print("Writing..")
					cv2.imwrite(f"Detected/image_{count}_orig.jpeg",img)
					cv2.imwrite(f"Detected/image_{count}_clean.jpeg",clean_plate)
					#cv2.imwrite("Detected/"+text+".jpeg",clean_plate)
					cv2.waitKey(0)
...
if __name__ == '__main__':
	...
	for count in range(316):
		print(f"Starting {count}..")
		print ("DETECTING PLATE . . .")

		try:
			img = cv2.imread(f"Indian_images/image_{count}.jpe")
